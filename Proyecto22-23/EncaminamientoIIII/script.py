...
for i in range(int(min_i),int(max_i)+1):
    ...
    process = Popen(["./SimRedMMkk","-s",str(seed),"-q",str(q),"-t",str(t),"-c", "-n 4", "config/config"+str(i)+".cfg"], stdout=PIPE)
    (output, err) = process.communicate()
    exit_code = process.wait()

    os.rename("config/config"+str(i)+".cfg.4k.out", "config_out/config"+str(i)+".cfg.4k.out")

    ##################################################
    #### Guardamos los valores en el fichero plot ####
    ##################################################

    with open('config_out/config'+str(i)+'.cfg.4k.out', 'r') as f:
        lines = f.readlines()
