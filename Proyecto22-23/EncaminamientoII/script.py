...
def create_plot_files():
    ...
    with open('basic_config.gnuplot', 'r') as src_file, open('gnuplot/config.gnuplot', 'w') as dest_file:
        ...
        dest_file.write("set title 'Encaminamiento 2'\n")
        ...
        dest_file.write("plot 'trafico_c.plot' with yerrorlines linecolor rgb 'yellow' lw 2 title 'Tráfico C', 'trafico_d.plot' with yerrorlines linecolor rgb 'green' lw 2 title 'Tráfico D', \
                        ...
                        'trafico_g.plot' with yerrorlines linecolor rgb 'black' lw 2 title 'Tráfico G' \n")
        dest_file.write("pause 1\n")
...
def generate_plot(A, data):
    ...
    bloqueo.append(list(data.items())[0]) ## Corresponde con el promediador a Trafico A
    bloqueo.append(list(data.items())[1]) ## Corresponde con el promediador b Trafico B
    bloqueo.append(list(data.items())[2]) ## Corresponde con el promediador c Trafico C
    bloqueo.append(list(data.items())[3]) ## Corresponde con el promediador d Trafico D
    bloqueo.append(list(data.items())[4]) ## Corresponde con el promediador e Trafico E

    bloqueo2.append(list(data.items())[4]) ## Corresponde con el promediador e Trafico F
    bloqueo2.append(list(data.items())[2]) ## Corresponde con el promediador c Trafico G
    ...
    for bloqueo_trafico in bloqueo2:
        ...
        if (promediador == 'e'):
            ...
            with open('gnuplot/trafico_f.plot', 'a') as fw:
                fw.write(""+str(A/2)+" "+str(AC_med)+" "+str(AC_min)+" "+str(AC_max)+"\n")

        else:

            with open('gnuplot/trafico_g.plot', 'a') as fw:
                fw.write(""+str(A/2)+" "+str(AC_med)+" "+str(AC_min)+" "+str(AC_max)+"\n")
